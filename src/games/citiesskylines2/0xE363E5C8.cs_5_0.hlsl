...
void main(uint3 vThreadID : SV_DispatchThreadID) {
  ...
  if (r0.w != 0) {
    ...
    r3.zw = floor(r3.xy);
    r3.xy = frac(r3.xy);
    r4.xyzw = -r3.xyxy * float4(0.5, 0.5, 0.166666672, 0.166666672)
  ...
  r0.w = cmp(0 != cb1[12].x);
  if (r0.w != 0) {
    r1.xyz = r0.xyz * float3(5.55555582, 5.55555582, 5.55555582)
        + float3(0.0479959995, 0.0479959995, 0.0479959995);
    r1.xyz = max(float3(0, 0, 0), r1.xyz);
    r1.xyz = log2(r1.xyz);
    r0.xyz = saturate(
        r1.xyz * float3(0.0734997839, 0.0734997839, 0.0734997839)
        + float3(0.386036009, 0.386036009, 0.386036009));
  } else {
    r0.w = cmp(0 != cb1[6].w);
    if (r0.w != 0) {
      r1.xyz = cb1[6].zzz * r0.xyz;
      r1.xyz = r1.xyz * float3(5.55555582, 5.55555582, 5.55555582)
          + float3(0.0479959995, 0.0479959995, 0.0479959995);
      r1.xyz = max(float3(0, 0, 0), r1.xyz);
      r1.xyz = log2(r1.xyz);
      r1.xyz = saturate(
  ...
  if (RENODX_SWAP_CHAIN_OUTPUT_PRESET != 0.f && RENODX_TONE_MAP_TYPE != 0.f) {
    r0.xyz = renodx::draw::ToneMapPass(untonemapped, r0.xyz);
  }
  u0[vThreadID.xyz] = r0.xyzx;
}
